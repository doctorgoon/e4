            ...
            <div class="card">
                <div class="card-head style-{{ Config::get('status_color.' . $call->status) }}">
                    <h3 style="padding-left: 20px;">
                        ...
                        @if ( ! is_null($call->client_id))
                            <a href="{{ action('ClientsController@showClient', [$call->client_id]) }}" style="padding-top: 12px; padding-right: 30px" class="btn btn-icon pull-right">
                            <i class="glyphicon glyphicon-user" style="display: inline; font-size: 30px; line-height: 0px;"></i></a>
                        @endif


                        <a href="{{ action('CallsController@destroyCall', [$call->id]) }}" style="padding-top: 10px; padding-right: 30px" class="btn btn-icon pull-right">
                            <i class="glyphicon glyphicon-trash" style="display: inline; font-size: 25px; line-height: 0px;"></i></a>

                        <a href="{{ action('CallsController@editCall', [$call->id]) }}" style="padding-top: 10px; padding-right: 30px" class="btn btn-icon pull-right">
                ...
                <div class="card-body">
                                        ...
                                        <li class="tile">
                                            <div class="tile-content">
                                                <div class="tile-text">
                                                    ...
                                                    <a href="{{ action('CallsController@editTicket', [$ticket->id])  }}" class="btn-icon">
                    ...
                    <div class="row">
                        ...
                        <a href="{{ action('CallsController@addTicket', [$call->id])  }}" class="btn ink-reaction btn-{{ Config::get('call_status.' . $call->status) }}">Ajouter un ticket</a>

                        <a href="{{ action('CallsController@setStatus', [$call->id]) }}" class="btn ink-reaction btn-{{ Config::get('call_status.' . $call->status) }}">Modifier le statut de l'appel</a>

                        @if(empty($call->client_id))
                            <a href="{{ action('CallsController@pairCall', [$call->id]) }}" class="btn ink-reaction btn-{{ Config::get('call_status.' . $call->status) }}">Associer à un client</a>
                        @else
                            <a href="{{ action('CallsController@pairCall', [$call->id]) }}" class="btn ink-reaction btn-{{ Config::get('call_status.' . $call->status) }}">Modifier le client associé</a>
                        @endif
