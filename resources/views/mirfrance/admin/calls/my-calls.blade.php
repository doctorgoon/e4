...
@section('title') Mes appels téléphoniques <a class="btn ink-reaction btn-floating-action btn-primary btn-fixed-right" href="{{ action('AdminCallsController@addCall') }}"><i class="glyphicon glyphicon-plus"></i></a><br>@endsection
                                ...
                                <div class="card panel">
                                        ...
                                        <div class="tools">
                                            <div class="btn-group">
                                                ...
                                                @if(!empty($call->client_id))
                                                    <a href="{{ action('AdminClientsController@showClient', [$call->client_id]) }}" class="btn btn-icon-toggle">
                                                ...
                                                <ul class="dropdown-menu animation-dock pull-right menu-card-styling" role="menu" style="text-align: left;">
                                                    <li><a href="{{ action('AdminCallsController@editCall', [$call->id]) }}">Modifier les informations</a></li>
                                                    <li><a href="{{ action('AdminCallsController@setStatus', [$call->id]) }}">Modifier le statut de l'appel</a></li>
                                                    @if(empty($call->client_id))
                                                        <li><a href="{{ action('AdminCallsController@pairCall', [$call->id]) }}">Associer à un client</a></li>
                                                    @else
                                                        <li><a href="{{ action('AdminCallsController@pairCall', [$call->id]) }}">Modifier le client associé</a></li>
                                                    @endif
                                                </ul>
                                            </div>
                                    ...
                                    <div id="accordion-{{ $call->id }}" class="collapse">
                                        <div class="card-body">
                                            ...
                                            <br /><div><a href="{{ action('AdminCallsController@addTicket', [$call->id])  }}" class="btn ink-reaction btn-flat btn-lg btn-{{ $colors_status[$call->status] }}">Ajouter un ticket</a></div><br>
                                                    ...
                                                    <div class="col-lg-12">
                                                        <div class="card">
                                                            <div class="card-head style-{{ Config::get('status_color.' . $call->status)] }}" style="padding-left: 10px;">
                                                                ...
                                                                <div><h4><a href="{{ action('AdminCallsController@editTicket', [$ticket->id])  }}" style="padding-left: 810px; text-decoration: none">Modifier</a></h4></div>
                                                            </div>
                                                            <div class="card-body">
